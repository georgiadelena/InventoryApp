...
public class ProductProvider extends ContentProvider {
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        String prodName = values.getAsString(ProductEntry.COLUMN_PRODUCT_NAME);
        ...
        String supName = values.getAsString(ProductEntry.COLUMN_SUPPLIER_NAME);
        if (supName == null || supName.length()==0) {
            throw new IllegalArgumentException("Supplier requires a name");
        }

        // Check that the supplier phone number is not null
        String supNumber = values.getAsString(ProductEntry.COLUMN_SUPPLIER_PHONE_NR);
        if (supNumber == null || supNumber.length()==0) {
    ...
    private int updateProduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If the {@link ProductEntry#COLUMN_SUPPLIER_PHONE_NR} key is present,
        // check that the value is not null.
        if (values.containsKey(ProductEntry.COLUMN_SUPPLIER_PHONE_NR)) {
            String supNumber = values.getAsString(ProductEntry.COLUMN_SUPPLIER_PHONE_NR);
            if (supNumber == null || supNumber.length()==0) {
                throw new IllegalArgumentException("Supplier requires a phone number");
            }
